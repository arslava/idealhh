...
// Confirmed via live fetch of /ar/careers/ — same underlying WPForms
// caregiver-application form as EN/RU, with English field labels
// throughout (unlike ES, which had a translated "Ciudad" label and a
// Spanish state dropdown — no such Arabic-specific translation exists
// here). CDPAP button (present in source alongside "Patient Enrollment")
// dropped per standing rule.
// A stray "UTM GCLID Source" field appeared in the fetch between
// Certification Type and Preferred Language — like the other forms' stray
// fields, this looks like a hidden tracking field rather than a real
// user-facing one; omitted pending confirmation.
const LANGUAGES = [
...
export default function CaregiverApplicationFormAr() {
  ...
  if (status === "success") {
    return (
      <div className="text-center py-8">
        <p className="text-xl font-semibold text-navy-800">شكراً لتقديم طلبك!</p>
        <p className="mt-2 text-grey-800">سيتواصل معك أحد أعضاء فريقنا قريباً.</p>
      </div>
    );
  }

  return (
    <form onSubmit={handleSubmit} className="text-left space-y-5" dir="ltr">
