...
export default function EsAboutUsPage() {
  ...
  return (
    <>
      {/* Two-column hero: text left, full-height image right, no rounding —
          matches the corrected pattern (see EN About Us, the verified
          reference for this layout) rather than the centered/WaveDivider
          layout used earlier for RU. */}
      ...
      <section className="mx-auto max-w-[1140px] px-4 py-16 grid sm:grid-cols-2 gap-6">
        {officeCards.map((office) => (
          <div key={office.title} className="rounded-3xl bg-white border border-navy-900/10 p-8 shadow-[0_20px_40px_rgba(56,75,116,0.08)]">
            ...
            <p className="mt-2 text-grey-800">{office.phoneLine}</p>
      ...
      {/* Language slider — source has 4 slides (EN/RU/ES/ZH), each with its
          ...
          the source rather than only showing the Spanish one. */}
      <section className="bg-navy-900 text-white">
