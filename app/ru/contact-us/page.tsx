          ...
          <div className="flex flex-col-reverse md:flex-row gap-10 items-center">
            <div className="md:w-1/2">
              <h1 className="font-display text-[2.8125rem] md:text-[3.75rem] leading-[1.05] font-black">
                {hero.title.replace(hero.highlight, "")}<span className="font-black">{hero.highlight}</span>
              </h1>
              <div className="mt-6 space-y-4">
                ...
                {hero.links.map((l) => (
                  <p key={l.href} className="text-[1.125rem] text-white/90 leading-relaxed">
                    {l.prefix}<Link href={l.href} className="text-accent font-semibold underline">{l.label}</Link>
                  </p>
                ))}
              </div>
            </div>
            <div className="md:w-1/2 flex justify-center">
              <div className="w-full max-w-[510px] rounded-[10px] bg-white shadow-[0_20px_40px_rgba(56,75,116,0.1)] p-11 text-left">
                <h2 className="font-display text-2xl font-semibold text-navy-800 text-center">Свяжитесь с нами</h2>
                <div className="mt-6"><ContactFormRu /></div>
              </div>
            </div>
          </div>
        </div>
