    ...
    <>
        ...
        <div className="mx-auto max-w-[1140px] px-4 grid lg:grid-cols-2 gap-10 items-stretch">
          <div className="py-16 lg:py-20">
            ...
            <div className="mt-8 grid sm:grid-cols-3 gap-4 max-w-xl">
              {hero.badges.map((b) => (
                <div key={b.bold} className="rounded-2xl bg-white/10 p-4">
                  <p className="font-display text-lg font-black text-accent">{b.bold}</p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-6">
              ...
              <a href="tel:7185172424" className="text-accent font-semibold">{hero.phone}</a>
            </div>
          </div>
          <div className="relative min-h-[280px] h-full">
            <Image src={hero.image} alt={hero.title} fill className="object-cover rounded-t-[49px] lg:rounded-none" />
          </div>
        </div>
      </section>
      <WaveDivider />
      ...
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[1140px] px-4 py-16">
          <h2 className="font-display text-[2rem] md:text-[2.5rem] leading-[1.1] font-black text-navy-800 text-center">{included.title}</h2>
          ...
          <div className="mt-8 text-center"><Button href={included.button.href}>{included.button.title}</Button></div>
        </div>
      </section>
      <section className="mx-auto max-w-[1140px] px-4 py-16">
        <h2 className="font-display text-[2rem] md:text-[2.5rem] leading-[1.1] font-black text-navy-800 text-center">{extras.title}</h2>
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <section className="bg-navy-900 text-white">
        <div className="mx-auto max-w-[1140px] px-4 py-16">
          <h2 className="font-display text-[2rem] md:text-[2.5rem] leading-[1.1] font-black text-center">{steps.title}</h2>
          ...
          <div className="mt-10 text-center"><Button href={steps.button.href} variant="secondary">{steps.button.title}</Button></div>
        </div>
      </section>
      <section className="mx-auto max-w-[1140px] px-4 py-16">
        <h2 className="font-display text-[2rem] md:text-[2.5rem] leading-[1.1] font-black text-navy-800 text-center">{whyChoose.title}</h2>
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[700px] px-4 py-16 text-center">
          ...
          <h2 className="mt-3 font-display text-[2rem] md:text-[2.5rem] leading-[1.1] font-black text-navy-800">{quote.title}</h2>
        ...
        <div className="mx-auto max-w-[1140px] px-4 py-16 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {officeCards.map((office) => (
              <div key={office.title} className="rounded-3xl bg-white border border-navy-900/10 p-6 text-center shadow-[0_20px_40px_rgba(56,75,116,0.08)] flex flex-col items-center">
                ...
                <Button href={office.button.href} variant="outline" className="mt-4">{office.button.title}</Button>
              </div>
            ))}
          </div>
        </div>
