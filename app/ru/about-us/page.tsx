  ...
  return (
    <>
      <section className="bg-navy-900 text-white">
        <div className="mx-auto max-w-[840px] px-4 py-16 lg:py-20 text-center">
          <h1 className="font-display text-[2.8125rem] md:text-[4.0625rem] leading-[1.05]">{hero.title}</h1>
          <p className="mt-6 max-w-xl mx-auto text-[1.125rem] md:text-[1.375rem] text-white/80 leading-relaxed">{hero.description}</p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Button href={hero.primaryButton.href} variant="secondary">{hero.primaryButton.title}</Button>
            <Button href={hero.secondaryButton.href}>{hero.secondaryButton.title}</Button>
          </div>
        </div>
      </section>
      <WaveDivider />
      ...
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[1140px] px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mt-4 text-[1.125rem] text-grey-800 leading-relaxed italic">«{areasServed.quote}»</p>
      ...
      <section className="mx-auto max-w-[1140px] px-4 py-16 grid sm:grid-cols-2 gap-6">
        {officeCards.map((office) => (
          <div key={office.title} className="rounded-3xl bg-white border border-navy-900/10 p-8 shadow-[0_20px_40px_rgba(56,75,116,0.08)]">
            ...
            <p className="mt-2 text-grey-800">Телефон: {office.phone}</p>
            <Button href={office.button.href} className="mt-6" variant="outline">{office.button.title}</Button>
          </div>
        ))}
      </section>

      {/* Language slider — source has 4 slides (EN/RU/ES/ZH), each with its
          own headline/description/button, kept exactly as they appear in
          the source rather than only showing the Russian one. */}
      <section className="bg-navy-900 text-white">
