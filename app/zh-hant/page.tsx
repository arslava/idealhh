...
import { zhHantHomePage } from "@/lib/content.zh-hant";
...
export const metadata = buildMetadata({
  key: "home",
  locale: "zhHant",
  title: "紐約市居家護理服務 | Ideal Home Health",
  // Real live meta-description, with its CDPAP mention removed per
  // standing rule (source reads "...家庭護理助理 (HHA)、個人護理助理
  // (PCA) 及 CDPAP。").
  description: "Ideal Home Health 在紐約市及五大行政區提供富有愛心的居家護理服務，包括家庭護理助理 (HHA) 及個人護理助理 (PCA)。您在紐約值得信賴的選擇。",
});

const icons = { heart: HeartHandshake, "heart-pulse": Stethoscope };

export default function ZhHantHome() {
  const { hero, servicesTeaser, whoBenefits, customPlans, locations, careers, prefooterCta, heroImage, locationsImage, locationCards } = zhHantHomePage;

  return (
    <>
      <JsonLd data={organizationSchema("zhHant")} />
      ...
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[1140px] px-4 py-20 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="order-1 md:order-2">
            ...
            <h3 className="mt-3 text-xl font-semibold text-navy-700">{customPlans.subtitle}</h3>
