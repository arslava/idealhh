...
import { zhHantTestimonialsPage } from "@/lib/content.zh-hant";
import { buildMetadata } from "@/lib/metadata";

// Real title confirmed via live fetch. The live meta-description itself
// has a rendering glitch (a stray `",` fragment — see note in
// content.zh-hant.ts), too broken to reuse for search results, so a real
// descriptive one is used here instead, same approach already taken for
// ES/AR's thin descriptions on this same page across locales.
export const metadata = buildMetadata({
  key: "testimonials",
  locale: "zhHant",
  title: "推薦與評論 - Ideal Home Health",
  description: "閱讀病患、護理人員及家屬對 Ideal Home Health 的真實評價與推薦，我們是紐約市值得信賴的居家照護機構。",
});
...
export default function ZhHantTestimonialsReviewsPage() {
  const { hero, testimonialsLabel, testimonials, reviewsLabel, reviews, prefooterCta } = zhHantTestimonialsPage;
    ...
    <>
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[840px] px-4 py-16 text-center">
          ...
          {/* Genuine rendering glitch in the source — see note in content.zh-hant.ts */}
          <h2 className="mt-3 text-xl md:text-2xl text-navy-700 font-semibold">{hero.secondaryHeading}</h2>
        </div>
      </section>
          ...
          {testimonials.map((t) => (
            <div key={t.author} className="rounded-3xl bg-white border border-navy-900/10 overflow-hidden shadow-[0_20px_40px_rgba(56,75,116,0.08)] flex flex-col">
              ...
              <div className="p-8 flex flex-col flex-1">
                ...
                <p className="mt-4 text-[1.125rem] md:text-[1.375rem] text-grey-800 leading-relaxed flex-1">「{t.content}」</p>
                <div className="mt-4">
      ...
      <section className="bg-bg-light">
        <div className="mx-auto max-w-[1140px] px-4 py-16">
          <h2 className="font-display text-[3.125rem] lg:text-[4.375rem] leading-[1.05] font-black text-navy-800 text-center">{reviewsLabel}</h2>
