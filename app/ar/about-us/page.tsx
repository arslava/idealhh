...
import { arAboutUsPage } from "@/lib/content.ar";
import { buildMetadata } from "@/lib/metadata";

export const metadata = buildMetadata({
  key: "aboutUs",
  locale: "ar",
  title: "نبذة عنا | Ideal Home Health: وكالة الرعاية المنزلية الموثوقة في مدينة نيويورك",
  description: "تعرف على مهمة Ideal Home Health المتمثلة في توفير رعاية منزلية رحيمة في مدينة نيويورك. نحن وكالة رعاية منزلية موثوقة تخدم بروكلين وبرونكس ومانهاتن منذ عام 2013.",
});

export default function ArAboutUsPage() {
  const { hero, mission, areasServed, officeCards, languages, statistics, prefooterCta } = arAboutUsPage;
  ...
  return (
    <>
      ...
      {/* Language slider — source has 4 slides (EN/RU/ES/ZH), none in
          Arabic — kept exactly as they appear in the source rather than
          fabricating an Arabic slide that doesn't exist there. */}
      <section className="bg-navy-900 text-white">
        <div className="mx-auto max-w-[1140px] px-4 py-16">
          <div className="grid md:grid-cols-2 gap-x-12 gap-y-10">
            {languages.map((lang) => (
              <div key={lang.title} dir="ltr" className="text-left">
                ...
                <Button href={lang.button.href} variant="secondary" className="mt-5">
                  <span dir="rtl">{lang.button.title}</span>
                </Button>
              </div>
            ))}
          </div>
