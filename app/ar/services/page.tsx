...
import { arServicesPage } from "@/lib/content.ar";
import { buildMetadata } from "@/lib/metadata";

export const metadata = buildMetadata({
  key: "services",
  locale: "ar",
  title: "خدمات الرعاية الصحية المنزلية في نيويورك | Ideal Home Health",
  description: "مساعدو صحة منزلية وممرضات زائرات مرخصات في مدينة نيويورك. رعاية رحيمة ومخصصة للمرضى في بروكلين وبرونكس.",
});

export default function ArServicesPage() {
  const { hero, services, whyChoose, prefooterCta } = arServicesPage;
      ...
      <section className="mx-auto max-w-[1140px] px-4 py-16 grid md:grid-cols-2 gap-8 items-stretch">
        {services.map((service) => (
          <div key={service.title} className="flex flex-col h-full rounded-3xl border border-navy-900/10 bg-white shadow-[0_20px_40px_rgba(56,75,116,0.08)] overflow-hidden">
            ...
            <div className="p-8 flex flex-col flex-1" dir="ltr">
              <h2 className="font-display text-2xl font-semibold text-navy-800 leading-snug text-left">{service.title}</h2>
              <p className="mt-4 text-[1.125rem] md:text-[1.375rem] text-grey-800 leading-relaxed flex-1 text-left">{service.description}</p>
              <Button href={service.button.href} variant="outline" className="mt-6 self-start">
                <span dir="rtl">{service.button.title}</span>
              </Button>
            </div>
          </div>
        ))}
