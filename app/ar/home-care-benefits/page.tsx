...
import { arBenefitsPage } from "@/lib/content.ar";
import { buildMetadata } from "@/lib/metadata";

export const metadata = buildMetadata({
  key: "homeCareBenefits",
  locale: "ar",
  title: "مزايا الرعاية المنزلية التي يغطيها Medicaid وMedicare | Ideal Home Health",
  description: "تعرف على ما إذا كنت أنت أو أحد أفراد أسرتك مؤهلاً للحصول على رعاية منزلية مجانية من خلال Medicaid وMedicare في مدينة نيويورك.",
});

export default function ArBenefitsPage() {
  const { hero, qualify, included, includedImage, extras, stepsImage, steps, whyChoose, locations, officeCards, quote, prefooterCta } = arBenefitsPage;
        ...
        <div className="mx-auto max-w-[1140px] px-4 grid lg:grid-cols-2 gap-10 items-stretch">
          <div className="py-16 lg:py-20">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-6">
              ...
              <a href="tel:7185172424" className="text-accent font-semibold" dir="ltr">{hero.phone}</a>
            </div>
          </div>
          ...
          <div className="mt-10 grid md:grid-cols-2 gap-8">
            {steps.items.map((step) => (
              <div key={step.number} className="flex gap-5">
                <span className="font-display text-2xl font-black text-accent shrink-0" dir="ltr">{step.number}</span>
                <div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {officeCards.map((office) => (
            <div key={office.title} className="rounded-3xl border border-navy-900/10 bg-white p-8 shadow-[0_20px_40px_rgba(56,75,116,0.08)]">
              ...
              <p className="mt-1 text-sm text-navy-muted" dir="ltr">{office.subtitle}</p>
              <p className="mt-4 text-grey-800 leading-relaxed" dir="ltr">{office.address}</p>
              <p className="mt-2 text-grey-800" dir="ltr">{office.phone}</p>
